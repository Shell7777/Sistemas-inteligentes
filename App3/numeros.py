...
def browsefunc():
    ...
    filename = filedialog.askopenfilename(
        filetypes=ftypes, defaultextension='.jpg')
    global img
...
browsebutton = Button(f, text="Browse", background="white",
                      fg="black", command=browsefunc)
# Posicion del Boton.
        ...
        if self.b1 == "down":
            if self.xold is not None and self.yold is not None:
                event.widget.create_line(
                    self.xold, self.yold, event.x, event.y, smooth='true', width=7, fill='black')
                self.draw.line(
                    ((self.xold, self.yold), (event.x, event.y)), (0, 0, 0), width=7)
...
def prediction():
    json_file = open(
        'C:/Users/Miclovin/Documents/UPN/Ciclo de Verano/App3/lenet_model.json', 'r')
...
### cargando los pesos ###
    ...
    loaded_model.load_weights(
        "C:/Users/Miclovin/Documents/UPN/Ciclo de Verano/App3/lenet_weights1.h5")
...
quitButton = Button(f, text="Salir", background="white",
                    fg="red", command=f.quit)
